...
class PressReleaseFilter(django_filters.FilterSet):
    ...
    date_published_min = django_filters.DateTimeFilter(
        field_name="date_published", lookup_expr="gte"
    )
    date_published_max = django_filters.DateTimeFilter(
        field_name="date_published", lookup_expr="lte"
    )
    date_published = django_filters.DateFilter(
        field_name="date_published", lookup_expr="date"
    )

    # Filtering by exact match on foreign key IDs
    ministry = django_filters.ModelChoiceFilter(queryset=Ministry.objects.all())
    audience_type = django_filters.ModelChoiceFilter(
        queryset=AudienceType.objects.all()
    )
    category = django_filters.ModelChoiceFilter(queryset=Category.objects.all())

    # Filtering by foreign key name (useful for user-friendly APIs)
    ministry_name = django_filters.CharFilter(
        field_name="ministry__name", lookup_expr="icontains"
    )
    audience_type_name = django_filters.CharFilter(
        field_name="audience_type__name", lookup_expr="icontains"
    )
    ...
    )  # filters the title field directly
